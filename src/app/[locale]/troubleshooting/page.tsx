...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    title: t.cliparc.troubleshooting.meta.title,
    description: t.cliparc.troubleshooting.meta.description,
...
export default async function TroubleshootingPage({ params }: Props) {
  ...
  return (
    <>
      <Header locale={locale} currentPath="/troubleshooting" />

      <div className="min-h-screen bg-gray-900 text-white">
        <div className="max-w-4xl mx-auto px-8 py-16">
          <Link href={`/${locale}/support`} className="text-blue-400 hover:text-blue-300 mb-8 inline-block">
          ...
          <div className="space-y-8">
            <section className="bg-gray-800 rounded-xl p-8">
              ...
              <div className="my-8">
                <Image src="/accessibility-settings.webp" alt={t.cliparc.troubleshooting.cantPaste.imageAlt} width={800} height={600} className="rounded-lg border border-gray-700 w-full" />
              </div>
              ...
              <ol className="space-y-4 text-gray-300">
                {(["step1", "step2", "step3", "step4"] as const).map((step, i) => (
                  <li key={step} className="flex gap-4">
                    <span className="flex-shrink-0 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center font-semibold">{i + 1}</span>
